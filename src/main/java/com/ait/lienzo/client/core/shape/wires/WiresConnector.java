...
public class WiresConnector
{
    ...
    private IPrimitive<?> getControlPoint(final int index) {
        return getControlPoint(index, null);
    }

    private IPrimitive<?> getControlPoint(final Integer index,
                                          final Point2D location) {
        final IControlHandleList handles = getPointHandles();
        for (int i = 0; i < handles.size(); i++) {
            final IControlHandle handle = handles.getHandle(i);
            if (null != index) {
                if (index == i) {
                    return handle.getControl();
                }
            } else {
                if (handle.getControl().getX() == location.getX() &&
                        handle.getControl().getY() == location.getY()) {
                    return handle.getControl();
                }
            }
        }
