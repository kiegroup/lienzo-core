...
public class TextTruncateWrapper extends TextNoWrap {
    ...
    private double getWrapBoundariesWidth() {
        ...
        return this.wrapBoundaries.getWidth() - margin;
    }
    ...
    private double[] calculateWrapBoundaries() {
        ...
        while (!hasVerticalSpace(numOfLines, lineHeight, wrapBoundaries.getHeight() - (Y_OFFSET * numOfLines))
                && numOfLines >= 0) {
            numOfLines--;
        }
    ...
    public void drawString(final Context2D context,
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (hasHorizontalSpaceToDraw(currentLine.toString(), currentWord, boundariesWidth)) {
                ...
                if (i != words.length - 1
                        && !hasVerticalSpace(lines.size() + 2, lineHeight, this.wrapBoundaries.getHeight() - (Y_OFFSET * lines.size() + 2))) {
                ...
                if (!hasVerticalSpace(lines.size() + 2, lineHeight, this.wrapBoundaries.getHeight() - (Y_OFFSET * lines.size() + 2))) {
                    if (currentLine.length() > 3) {
